...
example_path_for_tier() {
  local side="$1" tier="$2"
  if [[ "$side" == "op" ]]; then
    echo "$ROOT/benchmarks/results/large-aws-${tier}.example.json"
  else
    echo "$ROOT/benchmarks/results/tpuf-${tier}.example.json"
  fi
}
...
resolve_input_json() {
  ...
  if [[ "$DRY_RUN" == "1" ]]; then
    ...
    fallback="$(example_path_for_tier "$side" "$tier")"
    if [[ -f "$fallback" ]]; then
      echo "$fallback"
      return 0
    fi
