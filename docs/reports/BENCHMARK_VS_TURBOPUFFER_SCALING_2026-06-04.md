...
- **Dual-axis divergence:** query p50 **96 → 412 → 880 ms** (↑ with N); ingest **909 → 3571 → 758 docs/s** (peak @ 50k, WAL+index @ 100k) — see [§ Dual-axis insight](#dual-axis-insight-query-p50-vs-ingest-docss). **Write vs tpuf is meaningless** (commit latency vs end-to-end wall).
- **Write path (not comparable):** ingest docs/s **does not** track query scaling; tpuf ratios in this report are **query latency only**.
...
## Write throughput scaling (ingest — not comparable to tpuf writes)

> **Scope:** Every **turbopuffer ratio** elsewhere in this report (cold/warm p50 vs **874 / 14 ms**) is **query latency only**. turbopuffer’s published write path (**≤ ~200 ms** durable commit, **thousands of writes/s per namespace**) is a **different throughput model**—do **not** treat ingest docs/s here as a tpuf head-to-head.

**Ingest does *not* scale like query.** Cold query p50 grows **~9.2×** from 10k→100k (β ≈ 0.95, sub-linear per doc). Measured **effective ingest** (upsert + index-until-caught-up wall in `op-scaling-*.json`) **rises then collapses**: **909 → 3571 → 758 docs/s**. At **100k**, wall time is dominated by **~1 WAL commit/s per namespace** ([`COMPARISON.md`](../COMPARISON.md)) plus async index build on MinIO—not by vectors-per-second inside a commit window the way turbopuffer’s fleet write path is described.

| Tier | Docs | `ingest_wall_secs` | Effective docs/s | Scaling vs 10k |
|------|------|--------------------|------------------|----------------|
| 10k | 10,000 | **11** | **909** | 1.0× (baseline) |
| 50k | 50,000 | **14** | **3571** | **~3.9×** (5×10k batches; short wall, not sustained @ 100k class) |
| 100k | 100,000 | **132** | **758** | **~0.83×** vs 10k — **slower** despite 10× docs |

**Why 100k stalls at ~758 docs/s:** the harness uses **10k-row `upsert_columns` batches** with **~1.1 s spacing** between commits (WAL rate limit). For **100k** that implies **≥ ~10 s** of commit pacing alone, plus **index catch-up** (often **minutes** on MinIO in `bench_cold_100k_nightly`). Effective throughput ≈ `100_000 / (commit_pacing + index_wall)` → **~758 docs/s** on the committed refresh @ `7f7c0f5`—not “10× slower query ⇒ 10× slower ingest.”

| System | Write / ingest claim (published) | Comparable to this table? |
|--------|----------------------------------|---------------------------|
| **turbopuffer** | **≤ ~200 ms** durable commit; **thousands of writes/s per namespace** ([Tradeoffs](https://turbopuffer.com/docs/tradeoffs)) | **No** — latency bound + managed batching; no ~1 commit/s cap |
| **openpuffer** | **~1 WAL commit/s/ns**; table above is **end-to-end** ingest+index on MinIO | **No** — not tpuf write-path seconds |

Dashboard field: [`scaling-comparison-summary.json`](../../benchmarks/results/scaling-comparison-summary.json) → `ratios.ingest_docs_per_sec` (regenerated by `make bench-compare-tpuf`).

---

## Dual-axis insight: query p50 vs ingest docs/s

**Read both axes together:** cold **query** latency and **ingest** throughput answer different questions on this harness. Query p50 **monotonically rises** with document count; effective ingest docs/s **peaks at 50k** then **falls at 100k** when WAL pacing + index catch-up dominate—not a single “scaling law” for openpuffer.

| Tier | Docs (N) | Cold query **p50** (ms) | Trend vs 10k | Ingest **docs/s** | Trend vs 10k |
|------|----------|-------------------------|--------------|-------------------|--------------|
| 10k | 10,000 | **96** | 1.0× | **909** | 1.0× (baseline) |
| 50k | 50,000 | **412** | **4.3×** ↑ | **3571** | **3.9×** ↑ (**peak**) |
| 100k | 100,000 | **880** | **9.2×** ↑ | **758** | **0.83×** ↓ (below 10k) |

**Query axis (scales up with N):** more documents → more cold probe work per query → **96 → 412 → 880 ms** (β ≈ 0.95 on measured tiers). This is what we extrapolate toward 10M and compare to tpuf **874 ms** (query latency only).

**Ingest axis (peaks @ 50k, collapses @ 100k):** **909 → 3571 → 758 docs/s**. At **50k**, five 10k batches finish in **~14 s** wall (short index phase). At **100k**, **~1 WAL commit/s** ([`COMPARISON.md`](../COMPARISON.md)) plus **~132 s** index-until-caught-up caps throughput regardless of the query curve—**not** “10× docs ⇒ 10× slower ingest.”

### ASCII — two metrics vs doc count (same tiers)

```
cold p50 (ms)  ↑ with N          ingest docs/s  peak @ 50k, drop @ 100k
     900 ┤              ●880           4000 ┤      ▲3571
     400 ┤        ●412                 1000 ┤ ●909    ╲758
     100 ┤ ●96                              ┤         (WAL+index)
         └─10k──50k──100k→ N                  └─10k──50k──100k→ N
```

### Mermaid — side-by-side (same x-axis tiers)

```mermaid
xychart-beta
    title "Cold query p50 (ms) — rises with doc count"
    x-axis ["10k", "50k", "100k"]
    y-axis "p50 (ms)" 0 --> 900
    bar [96, 412, 880]
```
...
```mermaid
xychart-beta
    title "Ingest docs/s — peak @ 50k, WAL-limited @ 100k"
    x-axis ["10k", "50k", "100k"]
    y-axis "docs/s" 0 --> 4000
    bar [909, 3571, 758]
```

### Why comparing openpuffer ingest to turbopuffer writes is meaningless

| Dimension | openpuffer (this table) | turbopuffer (published) | Comparable? |
|-----------|-------------------------|-------------------------|-------------|
| **Metric** | End-to-end **ingest_wall_secs** (upsert batches + index until caught-up) | **≤ ~200 ms** durable **commit latency**; **thousands of writes/s per namespace** | **No** — latency bound vs wall-clock throughput |
| **Limiter @ 100k** | **~1 commit/s/ns** + MinIO index build | Managed WAL batching inside commit window | **No** — artificial pacing in OSS harness |
| **Use in tpuf report** | Explains **why 100k ingest stalls** while query p50 still “looks fine” vs 874 ms | Official ratios here are **cold/warm p50 only** | **Do not** publish “op is N× slower at writes than tpuf” from docs/s |

**Operator rule:** cite **query p50** (and warm p50) for any openpuffer vs turbopuffer ratio in this program; treat **ingest docs/s** as a **harness diagnostic** for WAL/index saturation, not a turbopuffer write-path benchmark.
